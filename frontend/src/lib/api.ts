...
export const getDeal = async (id: number): Promise<Deal> => {
  const { data } = await axiosInstance.get(`/api/deals/${id}`)
  return data
}
...
export const createDeal = async (dealData: {
  ...
  const { data } = await axiosInstance.post('/api/deals', dealData)
  return data
}

// Documents
export const getDocuments = async (dealId: number): Promise<Document[]> => {
  const { data } = await axiosInstance.get(`/api/documents/deal/${dealId}`)
  return data
}
...
export const uploadDocument = async (
  ...
  try {
    ...
    const { data: uploadData } = await axiosInstance.post('/api/documents/request-upload', uploadRequest)

    // Step 2: Upload file directly to R2 using presigned URL
    // Use direct axios for presigned URL (no auth needed)
    ...
    const { data } = await axiosInstance.post(`/api/documents/${uploadData.document_id}/confirm-upload`)
...
export const getDocumentDownloadUrl = async (documentId: number): Promise<{ download_url: string; filename: string }> => {
  const { data } = await axiosInstance.get(`/api/documents/${documentId}/download`)
  return data
}

// Analysis
export const startAnalysis = async (dealId: number) => {
  const { data } = await axiosInstance.post(`/api/deals/${dealId}/analysis/start`)
  return data
}

export const getWorkflows = async (dealId: number): Promise<Workflow[]> => {
  const { data } = await axiosInstance.get(`/api/deals/${dealId}/analysis/workflows`)
  return data
}

export const executeWorkflow = async (dealId: number, workflowId: number): Promise<Workflow> => {
  const { data } = await axiosInstance.post(`/api/deals/${dealId}/analysis/workflows/${workflowId}/execute`)
  return data
}

// Synthesis
export const generateSynthesis = async (dealId: number) => {
  const { data } = await axiosInstance.post(`/api/deals/${dealId}/synthesis/generate`)
  return data
}

export const getSynthesis = async (dealId: number): Promise<Synthesis> => {
  const { data} = await axiosInstance.get(`/api/deals/${dealId}/synthesis`)
  return data
}
